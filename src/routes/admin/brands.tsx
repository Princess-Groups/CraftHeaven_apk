...
function Brands() {
  ...
  async function add() {
    ...
    setName(""); qc.invalidateQueries({ queryKey: ["brands"] });
  }
  return (
    <div className="space-y-4 max-w-2xl">
      <h1 className="text-xl font-bold">Brands</h1>
      <div className="flex gap-2">
        <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Brand name" className="flex-1 rounded-lg border border-slate-200 px-3 py-2 text-sm" />
      ...
      <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        ...
        {!data?.length && <div className="p-6 text-center text-xs text-slate-400">No brands</div>}
      </div>
    </div>
  );
}
