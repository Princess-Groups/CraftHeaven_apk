...
function Users() {
  ...
  async function setRole(userId: string, role: string) {
    ...
    toast.success("Role granted"); qc.invalidateQueries({ queryKey: ["users-roles"] });
  }
            ...
            {(data ?? []).map((u, i) => (
              <tr key={u.id} className="border-t border-slate-100">
                ...
                <td className="p-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <div className="text-[10px] text-slate-500">{u.phone || u.id.slice(0, 8)}</div>
                    </div>
                  </div>
                </td>
                <td className="p-3">
                  <div className="flex flex-wrap gap-1 justify-center">
                    {u.roles.map((r) => (
                      <button key={r.id} onClick={() => removeRole(r.id)}
                ...
                <td className="p-3 text-center">
                  <select onChange={(e) => { if (e.target.value) { setRole(u.id, e.target.value); e.target.value = ""; } }} className="rounded-lg border border-slate-200 bg-white px-2 py-1 text-xs">
                    <option value="">+ Grant role</option>
                    {ROLES.filter((r) => !u.roles.some((x) => x.role === r)).map((r) => <option key={r} value={r}>{r}</option>)}
                  </select>
                </td>
              </tr>
            ))}
