  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [ordersR, itemsR, productsR, profilesR] = await Promise.all([
        supabase
          .from("orders")
          .select("id,total,channel,status,created_at,user_id")
          .gte("created_at", start12mo),
        supabase
          .from("order_items")
          .select("product_id,product_name,quantity,line_total,order_id,orders!inner(created_at)")
          .gte("orders.created_at", start30),
      ...
      const pending = orders.filter((o) =>
        ["NEW", "PROCESSING", "PACKED", "OUT_FOR_DELIVERY"].includes(o.status),
      ).length;
      ...
      return {
        ...
        totalCustomers: profilesR.count ?? 0,
        lowStock,
        outOfStock,
        pending,
        completed,
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-xs text-muted-foreground">Real-time overview of your store</p>
      </div>

      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <Kpi
          label="Today's Sales"
          value={inr(d?.todaySales ?? 0)}
          icon={IndianRupee}
          tone="emerald"
        />
        <Kpi
          label="Today's Orders"
          value={d?.todayOrdersCount ?? 0}
          icon={ShoppingBag}
          tone="peach"
        />
...
function Kpi({
  ...
  const tones: Record<string, string> = {
    ...
    slate: "bg-secondary-soft text-foreground/90",
  };
  return (
...
function ProductList({
  ...
  if (!items.length)
    return (
      <div className="py-8 text-center text-xs text-muted-foreground/70">No sales data yet</div>
    );
  return (
    <ul className="divide-y divide-border">
      {items.map((p, i) => (
        <li key={i} className="flex items-center gap-3 py-2.5">
          <div className="grid h-8 w-8 place-items-center rounded-lg bg-muted text-xs font-bold text-muted-foreground">
            {i + 1}
          </div>
