...
function Orders() {
  ...
  const { data: orders } = useQuery({
    queryKey: ["admin-orders", channelFilter, statusFilter],
    queryFn: async () => {
      let q = supabase
        .from("orders")
        .select(
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      inStore: all.filter((o) => o.channel === "IN_STORE").length,
      delivered: all.filter((o) => o.status === "DELIVERED").length,
      pending: all.filter((o) => ["NEW", "PROCESSING", "PACKED", "OUT_FOR_DELIVERY"].includes(o.status)).length,
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <div className="rounded-xl border border-border bg-white p-3 shadow-sm">
          <div className="text-[11px] uppercase text-muted-foreground">Total Orders</div>
          <div className="text-lg font-bold text-foreground">{stats.total}</div>
        </div>
        <div className="rounded-xl border border-border bg-white p-3 shadow-sm">
          <div className="text-[11px] uppercase text-muted-foreground">Pending</div>
        ...
        <div className="rounded-xl border border-border bg-white p-3 shadow-sm">
          ...
          <div className="text-lg font-bold text-emerald-600">{stats.delivered}</div>
        </div>
        <div className="rounded-xl border border-border bg-white p-3 shadow-sm">
          <div className="text-[11px] uppercase text-muted-foreground">Revenue</div>
          <div className="text-lg font-bold text-foreground">₹{stats.totalRevenue.toFixed(0)}</div>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="flex rounded-lg border border-border bg-white p-1 text-xs font-semibold">
          {(["ALL", "ONLINE", "IN_STORE"] as const).map((t) => (
            <button
              key={t}
              onClick={() => setChannelFilter(t)}
              className={`px-4 py-1.5 rounded ${channelFilter === t ? "bg-secondary text-white" : "text-muted-foreground"}`}
            >
              {t === "ALL" ? "All Orders" : t === "ONLINE" ? "Online" : "In-Store"}
            </button>
          ))}
        </div>
        <select
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
                ...
                <tr className="border-t border-border hover:bg-muted/30">
                  ...
                  <td className="p-3">
                    <span className={`inline-flex rounded-full px-2 py-0.5 text-[10px] font-semibold ${
                      o.channel === "ONLINE" ? "bg-sky-100 text-sky-700" : "bg-amber-100 text-amber-700"
                    }`}>
                  ...
                  <td className="p-3 text-xs">
                    {o.channel === "ONLINE" ? (
                      <span className="font-medium text-foreground">
                        ...
                        {o.user?.phone ? (
                          <span className="block text-[10px] text-muted-foreground">{o.user.phone}</span>
                        ) : null}
                      </span>
                    ) : (
                  ...
                  <td className="p-3 text-right text-muted-foreground">₹{Number(o.gst_total).toFixed(2)}</td>
                  <td className="p-3 text-right text-muted-foreground">₹{Number(o.shipping_charges).toFixed(2)}</td>
                  <td className="p-3 text-right font-semibold">₹{Number(o.total).toFixed(2)}</td>
                  <td className="p-3 text-center text-xs">
                    <span className="rounded bg-secondary-soft px-2 py-0.5">{o.payment_method}</span>
                  </td>
                  <td className="p-3">
                    <select
                      value={o.status}
                  ...
                  <tr className="bg-muted/70">
                    <td colSpan={12} className="p-4">
                      <div className="grid gap-4 md:grid-cols-2">
                        <div>
                          <div className="mb-1 text-[10px] font-semibold uppercase text-muted-foreground">Items</div>
                            ...
                            <tbody>
                              {(o.order_items ?? []).map((it, k) => (
                                <tr key={k} className="border-b border-border">
                                  ...
                                  <td className="py-1 text-right font-semibold">₹{Number(it.line_total).toFixed(2)}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
                        ...
                        <div>
                          <div className="mb-1 text-[10px] font-semibold uppercase text-muted-foreground">Bill summary</div>
                          <div className="space-y-1 text-xs">
                            <div className="flex justify-between"><span>Subtotal</span><span>₹{Number(o.subtotal).toFixed(2)}</span></div>
                            {Number(o.discount) > 0 && (
                              <div className="flex justify-between text-rose-600"><span>Discount</span><span>–₹{Number(o.discount).toFixed(2)}</span></div>
                            )}
                            {Number(o.gst_total) > 0 && (
                              <div className="flex justify-between"><span>GST</span><span>₹{Number(o.gst_total).toFixed(2)}</span></div>
                            )}
                            {Number(o.delivery_fee) > 0 && (
                              <div className="flex justify-between"><span>Delivery Fee</span><span>₹{Number(o.delivery_fee).toFixed(2)}</span></div>
                            )}
                            <div className="flex justify-between"><span>Shipping</span><span>₹{Number(o.shipping_charges).toFixed(2)}</span></div>
                            ...
                            <div className="flex justify-between"><span>Payment</span><span>{o.payment_method}</span></div>
                            {o.transaction_state && (
                              <div className="pt-1 text-[10px] text-muted-foreground">State: {o.transaction_state}</div>
                            )}
                          </div>
                        </div>
                      </div>
            ...
            {!filteredOrders.length && (
              <tr>
                <td colSpan={12} className="p-8 text-center text-xs text-muted-foreground/70">
                  No orders found
                </td>
              </tr>
            )}
