...
function Suppliers() {
  ...
  const { data } = useQuery({ queryKey: ["sup"], queryFn: async () => (await supabase.from("suppliers").select("*").order("name")).data ?? [] });
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm grid grid-cols-2 lg:grid-cols-5 gap-2">
        ...
        <button onClick={add} className="col-span-2 lg:col-span-5 flex items-center justify-center gap-1 rounded-lg bg-slate-900 px-4 py-2 text-xs font-semibold text-white"><Plus className="h-3.5 w-3.5" /> Add Supplier</button>
      </div>
