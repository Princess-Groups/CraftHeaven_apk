...
function OrderDetail() {
  ...
  const events = [...(order.order_status_events ?? [])].sort((a: any, b: any) =>
              ...
              return (
                <div key={s.key} className="flex flex-1 flex-col items-center text-center">
                  ...
                  {i < pipeline.length - 1 ? (
                    <div
                      className={cn(
                        "mt-[-24px] h-0.5 w-full",
                        i < currentIdx ? "bg-secondary" : "bg-border",
                      )}
                    />
                  ) : null}
                </div>
          ...
          <div className="mt-3 space-y-2">
            {order.order_items?.map((it: any) => (
              <div key={it.id} className="flex justify-between text-sm">
                <span>
                  {it.product_name} × {Number(it.quantity)} {it.unit ?? ""}
                  {it.variation ? (
                    <span className="ml-1 rounded bg-secondary-soft px-1 py-0.5 text-[10px] font-semibold text-secondary">
                      {it.variation}
                    </span>
                  ) : null}
                </span>
            ...
            {order.tax_type === "CGST_SGST" ? (
              <>
                {Number(order.cgst_amount) > 0 ? (
                  <div className="flex justify-between text-sm">
                    ...
                    <span>₹{Number(order.cgst_amount).toFixed(0)}</span>
                  </div>
                ) : null}
                {Number(order.sgst_amount) > 0 ? (
                  <div className="flex justify-between text-sm">
                    <span>SGST (State)</span>
                    <span>₹{Number(order.sgst_amount).toFixed(0)}</span>
                  </div>
                ) : null}
              </>
              ...
              <div className="flex justify-between text-sm">
                <span>IGST</span>
                <span>₹{Number(order.igst_amount).toFixed(0)}</span>
              </div>
            ) : null}
            {Number(order.shipping_charges) > 0 ? (
              <div className="flex justify-between text-sm">
                <span>Shipping / Courier</span>
                <span>₹{Number(order.shipping_charges).toFixed(0)}</span>
              </div>
            ) : null}
            <div className="flex justify-between text-sm">
              <span>Delivery</span>
        ...
        <div className="rounded-2xl border border-border/60 bg-card p-5 shadow-card">
          ...
          <div className="mt-4 rounded-xl bg-secondary-soft p-3 text-sm">
            ...
            <div className="mt-1 text-xs text-muted-foreground">
              ...
              {order.notes && /UTR/.test(String(order.notes)) ? (
                <span className="ml-2 rounded-full bg-secondary-soft px-2 py-0.5 font-medium">
                  {order.notes}
                </span>
              ) : null}
            </div>
